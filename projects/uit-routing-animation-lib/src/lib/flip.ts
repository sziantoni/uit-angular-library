...
export const flipLeavePart = animation([
  query(
...
export const flipEnterPart = animation([
  query(':enter', [
    style({ opacity: 0})
  ], {optional: true} ),
  query(':enter' , [
...
export const flipEnterParams = animation([
...
export const flipLeaveParams = animation([
  query(':leave' , style({
    position: 'relative'
  }), { optional: true})
]);
...
const flipA = animation([
      useAnimation(flipEnterParams),
      useAnimation(flipLeaveParams),
      sequence([
        useAnimation(flipLeavePart),
        useAnimation(flipEnterPart)
])])
;
